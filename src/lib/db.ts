...
-- There is deliberately no table here counting rewrites.
--
-- The free weekly correction budget is metered in tokens, and a token count is
-- a measurement of the visitor's document. Storing one per account would mean
-- posting that measurement to this server on every rewrite, which would
-- contradict the guarantee that rewriting never leaves the device. The budget
-- is therefore counted in the browser's own storage instead (see
-- src/lib/entitlements/rewrite-budget-store.ts), and that trade is stated
-- there rather than quietly made here.
`
